...
export function applyConfigToProcessEnv(config, env = process.env) {
  const values = {
    NODE_ENV: config.nodeEnv,
    HOST: config.host,
    PORT: String(config.port),
    WR_CHAT_PUBLIC_ORIGIN: config.publicOrigin,
    WR_API_ORIGIN: config.wrApiOrigin,
    WR_CHAT_SHARED_SECRET: config.sharedSecret,
    WR_CHAT_ALLOWED_ORIGINS: config.allowedOrigins.join(","),
  };
